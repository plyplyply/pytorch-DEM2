...
def save_debug_images(
    ...
    batch_outputs_hm,
    prefix,
    batch_pred_tagmaps = None
):
    if not config.DEBUG.DEBUG:
        return

    num_joints = config.DATASET.NUM_JOINTS + 1
    batch_pred_offmaps = batch_outputs_off
    batch_pred_heatmaps = batch_outputs_hm
    ...
    if config.DEBUG.SAVE_OFFSETS_GT and batch_offsets is not None:
        file_name = '{}_off_gt.jpg'.format(prefix)
        save_batch_maps(
            batch_images, batch_offsets, batch_masks, file_name, 'tagmap'
        )
    if config.DEBUG.SAVE_OFFSETS_PRED:
        file_name = '{}_off_pred.jpg'.format(prefix)
        save_batch_maps(
            batch_images, batch_pred_offmaps, batch_masks, file_name, 'tagmap'
        )
    if config.DEBUG.SAVE_TAGMAPS_PRED and batch_pred_tagmaps is not None:
