...
class CocoDataset(Dataset):
    ...
    def _get_anno_file_name(self):
        ...
        if 'test' in self.dataset:
            return os.path.join(
                self.root,
                #'annotations',
                'image_info_{}.json'.format(  # test2017
                    self.dataset
                )
            )
        else:
            ...
            return os.path.join(
                self.root,
                #'annotations',
                'person_keypoints_{}.json'.format( # val2017
                    self.dataset
                )
            )
    ...
    def evaluate(self, cfg, preds, scores, output_dir,
        ...
        res_file = os.path.join(
            res_folder, 'keypoints_%s_results.json' % self.dataset)# val2017
