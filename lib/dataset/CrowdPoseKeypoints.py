...
# and Bin Xiao (leoxiaobin@gmail.com)
...
class CrowdPoseKeypoints(CrowdPoseDataset):
    def __init__(self,
        ...
        self.num_scales = self._init_check(heatmap_generator, joints_generator,offset_generator)

        self.num_joints = cfg.DATASET.NUM_JOINTS + 1
    ...
    def __getitem__(self, idx):
        ...
        for scale_id in range(self.num_scales):
            ...
            offset_t, offset_rev_t = self.offset_generator[scale_id](joints_list[scale_id])

            target_list.append(target_t.astype(np.float32))
            offset_list.append(offset_t.astype(np.float32))
            offset_rev_list.append(offset_rev_t.astype(np.float32))
            mask_list[scale_id] = mask_list[scale_id].astype(np.float32)
            joints_list[scale_id] = joints_t.astype(np.int32)

        return img, target_list, mask_list, joints_list, offset_list, offset_rev_list
    ...
    def _init_check(self, heatmap_generator, joints_generator, offset_generator):
        ...
        assert isinstance(offset_generator, (list,tuple)), 'offset_generator should be a list or tuple'
        ...
        assert len(heatmap_generator) == len(offset_generator), \
            'heatmap_generator and offset_generator should have same length,' \
            'got {} vs {}.'.format(
                len(heatmap_generator), len(offset_generator)
            )
