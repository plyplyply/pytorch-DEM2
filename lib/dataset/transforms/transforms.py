...
class RandomAffineTransform(object):
    ...
    def _get_affine_matrix(self, center, scale, res, rot=0):
        ...
        if not rot == 0:
            ...
            rot_rad = rot * np.pi / 180 # (np.random.random() * 2 - 1) * self.max_rotation
    ...
    def __call__(self, image, mask, joints):
        ...
        for i, _output_size in enumerate(self.output_size):
            ...
            '''
            for jr, joints_person in enumerate(joints[i]):
                joints_sum = np.zeros(2)  # (joints_person[:-1, :2], axis=0)
                num_vis_joints = 0
                for erj in joints_person[:-1, :2]:
                    if erj[0] >= 0 and erj[0] < _output_size and erj[1] >= 0 and erj[1] < _output_size:
                        
                        joints_sum[0] += erj[0]
                        joints_sum[1] += erj[1]
                        num_vis_joints += 1
                if num_vis_joints > 0:
                    joints[i][jr, -1, :2] = joints_sum / num_vis_joints
                    # joints[i][j, -1, 2] = 1###-1！
            '''
